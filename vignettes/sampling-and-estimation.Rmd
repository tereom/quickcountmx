...
mrp_gto <- mrp_estimation(gto_2012, pri_pvem:otros, frac = 0.06, 
    ...
    n_chains = 2, seed = 19291, parallel = TRUE)
```
...
We can also fit the count model for a single party:

```{r}
counts_fit <- mrp_party_estimation(gto_2012, party = prd, 
  stratum = distrito_loc_17, frac = 0.06, 
  seed = 212187, n_chains = 2, n_burnin = 1000, n_iter = 2000, model_string = "model_bern_t")
qplot(counts_fit$n_votes, binwidth = 1000) + 
  geom_vline(xintercept = sum(gto_2012$prd), colour ='red')
```

And examine the fit:

```{r}
class(counts_fit$fit)
plot(counts_fit$fit)
```
