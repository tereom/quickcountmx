...
```{r}
# devtools::install_github("tereom/quickcount")
library(quickcount)
library(tidyverse)
```
...
random sampling and stratified random sampling. For example if we want to 
select a simple random sample with 2% of the data one simply do:
...
We begin exemplifying ratio estimation, to compute estimations we using ratio
...
gto_strata_sizes <- gto_2012 %>%
    dplyr::group_by(distrito_loc_17) %>%
    dplyr::mutate(n_stratum = n())
```

Now lets suppose we have a stratified random sample with 4% of te data (pps):
...
```{r}
mrp_gto <- mrp_estimation(gto_2012, pri_pvem:otros, frac_sample = 0.06, 
    n_iter = 500, n_burnin = 250, n_chains = 2, seed = 19291)
```
...
plot(mrp_gto$jags_fits$pri_pvem)
```
