...
mrp_party_estimation_stan <- function(data, party, stratum, frac = 1,
                                 n_iter = 300, warmup = 150, n_chains = 2,
                                 seed = NA,
  ...
  data_full <- list(N_f = nrow(data), n_f = data$ln_total,
                    ...
                    x_f = x_full)
  stan_fit <- sampling(stanmodels[[model_string]], iter = n_iter, warmup = warmup,
                              chains = n_chains, data= c(data_sample, data_full),
                              init = "0",
                              cores = n_chains, control = list(max_treedepth = 15))
